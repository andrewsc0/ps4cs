...
No, not all the points fall on the line, there are some deviations. But the trend follows the line pretty closely, for the most part and it is safe to assume that this simulation (sim_norm) represents a bienak distribution. Not so suprisingly, this line is somewhat close to the actual data, seen above.
```{r}
qqnorm(sim_norm)
qqline(sim_norm)
```
...
pnorm(q = SixFtInCM, mean = maleMean, sd = maleSD)
```
```{r}
